...
public class IntroToEnums {
	...
	public static void main(String[] args) {
		...
		StatesOfMatter [] ar = StatesOfMatter.values(); 
		// 4. Ask the user for a state of matter.
		JOptionPane.showInputDialog("enter matter baby");
		// 5. Iterate through the array and find what the user entered.
		// Hint: .name() or .toString
		for (StatesOfMatter madder: ar) {
			System.out.println(madder.ordinal());
		}
		...
		StatesOfMatter m = ar[(int)(Math.random()* 4)];
		// 11. Print outs both of its temperatures.
		System.out.println(m.getTemp());
		System.out.println(m.convertToFarenheit(m.getTemp()));
		...
		switch (m) {
		case GAS:
			System.out.println("carbon dioxide emissions");
			break;
		case SOLID: 
			System.out.println("chicken pastrami");
			break;
		case LIQUID:
			System.out.println("coca cola zero zugar");
			break;
		}
